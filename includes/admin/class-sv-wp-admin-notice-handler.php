...
if ( ! class_exists( 'SV_WP_Admin_Notice_Handler' ) ) :
 ...
 * @since 1.1.0
 */
class SV_WP_Admin_Notice_Handler {


	/** @var SV_EDD_Plugin the plugin */
	private $plugin;
	...
	public function __construct( $plugin ) {
		...
		add_action( 'wp_ajax_wp_plugin_framework_' . $this->get_plugin()->get_id() . '_dismiss_notice', array( $this, 'handle_dismiss_notice' ) );
	}
	 ...
	 *     @type bool $add_settings_error      If the notice should be added to
	 *                                         the core WP settings notices and
	 *                                         be displayed after a redirect
	 * }
	 */
	public function add_admin_notice( $message, $message_id, $params = array() ) {
		...
		if ( $this->should_display_notice( $message_id, $params ) ) {
			...
			if ( $params['add_settings_error'] ) {
				add_settings_error( $this->get_plugin()->get_id() . '-notices', '', $message, $params['notice_class'] );
			}
		}
	}
	...
	public function render_admin_notices( $is_visible = true ) {
		...
		if ( $is_visible && ! self::$admin_notice_placeholder_rendered ) {
			...
			echo '<div class="js-wp-plugin-framework-admin-notice-placeholder"></div>';
			self::$admin_notice_placeholder_rendered = true;
		}

		// display any WordPress settings API errors
		if ( $is_visible ) {
			settings_errors( $this->get_plugin()->get_id() . '-notices' );
		}
	}
	...
	public function render_admin_notice( $message, $message_id, $params = array() ) {
		...
		$classes = array(
			'js-wp-plugin-framework-admin-notice',
			$params['notice_class'],
		);
		...
		if ( version_compare( get_bloginfo( 'version' ), '4.2', '>=' ) ) {
			...
			if ( $dismissible ) {
				...
				$dismiss_link = sprintf(
					'<a href="#" class="js-wp-plugin-framework-notice-dismiss" style="float: right;">%s</a>',
					esc_html__( 'Dismiss', 'jilt-for-edd' )
				);
	...
	public function render_admin_notice_js() {
		...
		jQuery(function($) {
			...
			$( '.js-wp-plugin-framework-admin-notice' ).on( 'click.wp-dismiss-notice', '.notice-dismiss', function( e ) {

				var $notice = $( this ).closest( '.js-wp-plugin-framework-admin-notice' );
			...
			$( 'a.js-wp-plugin-framework-notice-dismiss' ).click( function( e ) {
			...
			function log_dismissed_notice( pluginID, messageID ) {
				...
				$.get(
					ajaxurl,
					{
						action:    'wp_plugin_framework_' + pluginID + '_dismiss_notice',
						messageid: messageID
					}
				);
			}
			...
			$( '.js-wp-plugin-framework-admin-notice:hidden' ).insertAfter( '.js-wp-plugin-framework-admin-notice-placeholder' ).show();
		});
		</script>
		<?php
	}
	...
	public function dismiss_notice( $message_id, $user_id = null ) {
		...
		update_user_meta( $user_id, '_wp_plugin_framework_' . $this->get_plugin()->get_id() . '_dismissed_messages', $dismissed_notices );
		 ...
		 */
		do_action( 'wp_' . $this->get_plugin()->get_id(). '_dismiss_notice', $message_id, $user_id );
	}
	...
	public function get_dismissed_notices( $user_id = null ) {
		...
		$dismissed_notices = get_user_meta( $user_id, '_wp_plugin_framework_' . $this->get_plugin()->get_id() . '_dismissed_messages', true );
	 ...
	 *
	 * @since 1.1.0
	 * @return SV_EDD_Plugin returns the plugin instance
	 */
