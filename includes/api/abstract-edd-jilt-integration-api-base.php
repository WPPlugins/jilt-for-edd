 ...
 * @since 1.1.0
 */
abstract class EDD_Jilt_Integration_API_Base {
	...
	public function handle_api_request( $request ) {
		...
		$secret_key = $this->get_plugin()->get_integration()->get_secret_key();
		...
		if ( ! is_ssl() && apply_filters( 'edd_jilt_client_api_requires_ssl', true, $request ) ) {
		...
		unset( $request['hash'], $request['edd_action'] );
		...
		// verify hash
		$hash = hash_hmac( 'sha256', $request_query, $secret_key, true );
		...
		$max_age = apply_filters( 'edd_jilt_client_api_request_max_age', 5 * MINUTE_IN_SECONDS, $request );
		...
		} catch ( SV_EDD_Plugin_Exception $e ) {
			// request failed
	...
	protected function log_api_response( $body, $code ) {
		...
		$response_data = array(
			...
			'headers' => array(
				...
				'x-jilt-version' =>  $this->get_plugin()->get_version(),
			),
			'body' => $body,
		);

		$this->get_plugin()->get_logger()->debug( $this->get_plugin()->get_logger()->get_api_log_message( $response_data ) );
	}
	...
	protected function log_api_request( $request ) {
		...
		$this->get_plugin()->get_logger()->debug( preg_replace( '/^Request/', 'Incoming Request', $this->get_plugin()->get_logger()->get_api_log_message( $request_data ) ) );
	}
